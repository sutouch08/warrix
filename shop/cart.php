...
function getcondition(){
	if(checkboxes.checked){
		$("#checkcondition").html("<button class='btn btn-primary btn-lg' id='check_condition' width='50%' type='button' onclick='not()'  ><i class='fa fa-arrow-right'></i>&nbsp;ดำเนินการสั่งซื้อ( Check Out ) </button>");
	}else{
		$("#checkcondition").html("<button class='btn btn-primary btn-lg' id='check_condition' width='50%'disabled='disabled' ><i class='fa fa-arrow-right'></i>&nbsp; ดำเนินการสั่งซื้อ( Check Out ) </button>");
	}
}
function confrimorder() {
	...
	//alert(payment);
	$("#result").html("<h1>&nbsp;</h1><table style='width: 100%; border:0px;'><tr><td align='center'><i class='fa fa-spinner fa-spin fa-5x'></i><br/><h4>กำลังดำเนินการสั่งซื้อ (Order Processing) ......</h4></td></tr></table>");
		  $.ajax({
					type: 'GET',
					...
					success: function(data){
						...
						if(message == "not"){
							...
							dropproductcart(0,'0');
							$("#result").html("<div class='alert alert-success'>ดำเนินการสั่งซื้อเรียบร้อยแล้ว ( Process Completed )</div>");
						}
					}
				});
}
...
<div class='container main-container headerOffset'>
  ...
  <div class='row'>
    <div class='col-lg-9 col-md-9 col-sm-7'>
      <h1 class='section-title-inner'><span><i class='glyphicon glyphicon-shopping-cart'></i> ตะกร้าสินค้า( Cart ) </span></h1>
    </div>
    <div class='col-lg-3 col-md-3 col-sm-5 rightSidebar'>
      <h4 class='caps'><a href='index.php'><i class='fa fa-chevron-left'></i>&nbsp;ซื้อสินค้าต่อ (Continue Shopping) </a></h4>
    </div>
  </div><!--/.row-->
...
<div id='cartfull'><?php if(isset($_GET['error'])){
  ...
  <div style='clear:both'></div><?php }else{
	  if(isset($_GET['finish'])){
		  ...
		  $search = array("\n","\s");
		  $replace = array("<br>","&nbsp;");
	 	  echo "<div id='cartfull'><div class='alert alert-success'>ดำเนินการสั่งซื้อเรียบร้อยแล้ว ( Process Completed )</div></div><br>".str_replace($search,$replace,$value);
	  }else{
