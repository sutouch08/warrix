	...
	createCookie("id_cart","",-1);
		...
		$.ajax({
					...
					success: function(total)	{
						if(total != "null"){
							$('#sumtotalcartin').html("ราคารวม(Sub total) : "+total);
							$('#sumtotalcart').html("ตะกร้า : Cart("+total+")");
							$('#sumtotalcartin_for_mobile').html("ราคารวม : "+total);
							$('#sumtotalcart_for_mobile').html("ตะกร้า("+total+")");
						}else{
							$('#sumtotalcartin').html("ราคารวม (Sub total) : ");
							$('#sumtotalcart').html("ตะกร้า : Cart (ว่างเปล่า : Empty)");
							$('#sumtotalcartin_for_mobile').html("ราคารวม (Sub total) : "+total);
							$('#sumtotalcart_for_mobile').html("ตะกร้า : Cart ("+total+")");
						}
					}
				});
						}
