...
public function addEmployee(array $data){ ////เพิ่มพนักงานใหม่
	list($id_profile, $first_name, $last_name, $email, $password, $s_key, $active) = $data;
	if($password !=""){ $password = md5($password); }
	if(!$this->check_email($email)){
			$this->error_message ="มีอีเมล์นี้ในระบบแล้ว";
			return false;
		}else if(!$this->check_s_key($s_key)){
			$this->error_message = "รหัสลับซ้ำกับพนักงานคนอื่น";
			return false;
		}else{
			$sql = dbQuery("INSERT INTO tbl_employee (id_profile, first_name, last_name, email, password, last_login, date_register, s_key, active) 
					VALUES ($id_profile, '$first_name', '$last_name', '$email', '$password', NOW(), NOW(), '$s_key', $active)");
			if($sql){
				return true;
			}else{
				$this->error_message = "ไม่สามารถเพิ่มพนักงานได้";
				return false;
			}
		}
}
...
public function editEmployee(array $data){
	list($id_employee, $id_profile, $first_name, $last_name, $email, $password, $s_key, $active) = $data;
	if($password !=""){ $pass = " ,password = '".md5($password)."'"; }else{ $pass = ""; }
	if($s_key !=""){ 
		$key = ", s_key = '".md5($s_key)."'";
		if(!$this->check_s_key(md5($s_key), $id_employee)){
			$this->error_message = "รหัสลับซ้ำกับพนักงานคนอื่น";
			return false;
		}
	 }else{ $key = ""; }
	 if(!$this->check_email($email, $id_employee)){
		$this->error_message ="มีอีเมล์นี้ในระบบแล้ว";
			return false;
	 }else{	
		$sql = dbQuery("UPDATE tbl_employee SET id_profile = $id_profile, first_name = '$first_name', last_name = '$last_name', email = '$email' $pass $key, active = $active WHERE id_employee = $id_employee");
		if($sql){
			return true;
		}else{
			$this->error_message = "แก้ไขข้อมูลพนักงานไม่สำเร็จ";
			return false;
		}
	 }
}
...
public function reset_password(array $data){
	list($id_employee, $email, $password) = $data;
	if($password !=""){ $pass = ", password = '".md5($password)."'"; }else{ $pass = ""; }
	if(!$this->check_email($email, $id_employee)){
		$this->error_message = "อีเมล์ซ้ำ มีอีเมล์นี้ในระบบแล้ว";
		return false;
	}else{
		$sql = dbQuery("UPDATE tbl_employee SET email = '$email' $pass WHERE id_employee = $id_employee");
		if($sql){
			return true;
		}else{
