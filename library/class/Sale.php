...
class sale{
	...
	public function saleWeekTable(){
		...
		while($rs = dbFetchArray($sql)){
			...
			$d = 0;
			foreach($week as $day){
			$sqr= dbQuery("SELECT SUM(total_amount) FROM tbl_order_detail_sold WHERE id_sale = $id_sale AND id_role IN(1,5) AND date_upd LIKE'$day%'");
			list($res) = dbFetchArray($sqr);
			$net_res = $res/1.07;
			$sale_amount = 0;
	...
	public function saleLeaderBoard($from="", $to=""){ // แสดงเป็นเดือน
		...
		while($rs = dbFetchArray($sql)){
			...
			$sqm = dbQuery("SELECT first_name FROM tbl_sale LEFT JOIN tbl_employee ON tbl_sale.id_employee = tbl_employee.id_employee WHERE tbl_sale.id_employee = $id_employee");
			list($first_name) = dbFetchArray($sqm);
			$sqr = dbQuery("SELECT SUM(total_amount) AS total_amount FROM tbl_order_detail_sold WHERE id_sale = $id_sale AND id_role IN(1,5) AND (date_upd BETWEEN '$from 00:00:00.000000' AND '$to 23:59:59.000000')");
	...
	public function groupLeaderBoard($from="",$to=""){
		...
		while($rs = dbFetchArray($sql)){
			...
			$sqr = dbQuery("SELECT SUM(total_amount) AS total_amount FROM tbl_order_detail_sold LEFT JOIN tbl_customer ON tbl_order_detail_sold.id_customer = tbl_customer.id_customer WHERE id_default_group = $id_group AND id_role IN(1,5) AND (tbl_order_detail_sold.date_upd BETWEEN '$from 00:00:00.000000' AND '$to 23:59:59.000000')");
			$sale_amount = 0;
			list($amount) = dbFetchArray($sqr);
			$net_amount =  $amount/1.07;
			$sale_amount = $sale_amount + $net_amount;
	...
	public function sale_amount($option, $id_sale){
		...
		$sql = dbQuery("SELECT SUM(total_amount) AS total_amount FROM tbl_order_detail_sold WHERE id_role IN(1,5) AND id_sale = $id_sale AND (date_upd BETWEEN '$from 00:00:00.000000' AND '$to 23:59:59.000000')");
		$total_amount = 0;
		list($amount) = dbFetchArray($sql);
		$net_amount =  $amount/1.07;
