...
class order{
		...
		if($row>0){
			...
			while($i = dbFetchArray($sql)){
				...
				<td style='text-align:center; vertical-align:middle;'>
						<button type='button' id='edit".$i['id_order'].$i['id_product_attribute']."' class='btn btn-warning btn-sx' onclick='edit_product(".$i['id_order'].",".$i['id_product_attribute'].") ' $active $can_edit>
							<span class='glyphicon glyphicon-pencil' style='color: #fff;'></span>
						</button><button type='button' id='update".$i['id_order'].$i['id_product_attribute']."' onclick='update(".$i['id_order'].",".$i['id_product_attribute'].")' class='btn btn-default' style='display:none;' $active>Update</button>";
						if($this->valid==1 || $this->current_state==8){echo "";}else{echo"
					<a href='controller/".$content."Controller.php?delete=y&id_order=".$i['id_order']."&id_product_attribute=".$i['id_product_attribute']."'>";} echo"
						<button type='button' id='delete".$i['id_order'].$i['id_product_attribute']."' class='btn btn-danger btn-sx' onclick=\"return confirm('คุณแน่ใจว่าต้องการลบ ".$i['product_reference']." : ".$i['product_name']." : ".$i['barcode']." ? '); \" $active $can_delete>
	...
	public function getCurrentOrderAmount(){
		$id_order = $this->id_order;
		list($current_order_amount) = dbFetchArray(dbQuery("SELECT SUM(total_amount) AS amount FROM tbl_order_detail WHERE id_order = $id_order"));
		return $current_order_amount;
	}
