...
<!-- page place holder -->
<div class="row">
	<div class="col-xs-6"><h3><span class="glyphicon glyphicon-inbox"></span>&nbsp;<?php echo $page_name; ?></h3>
  </div>
    <div class="col-xs-6">
	  ...
	  if(!isset($_GET['id_order'])&&!isset($_GET['process'])){
		   echo"
		   <li><a href='index.php?content=prepare&process' style='text-align:center; background-color:transparent; padding-bottom:0px;' ><button type='button' class='btn btn-link' ><span class='glyphicon glyphicon-forward' style='color:#5cb85c; font-size:30px;'></span><br />กำลังจัด</button></a></li>";
	   }else{
		   echo"
		   <li><a href='index.php?content=prepare' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link'><span class='glyphicon glyphicon-arrow-left' style='color:#5cb85c; font-size:30px;'></span><br />กลับ</button></a></li>";
	   }
...
echo"
	...
	while($list = dbFetchArray($sql)){
			...
			list($prepared) = dbFetchArray(dbQuery("SELECT SUM(qty) FROM tbl_temp WHERE id_order = $id_order AND id_product_attribute = $id_product_attribute"));
...
echo "</td></tr>";
		...
		echo"<tr><td colspan='7' align='center'><a href='controller/prepareController.php?close_job&id_order=$id_order&id_employee=$id_user'><button type='button' class='btn btn-success' onclick=\"return check_cancal() \">จัดเสร็จแล้ว</button></a></td></tr>";
	}
	echo"
			</table>
			<hr style='border-color:#CCC; margin-top: 15px; margin-bottom:15px;' />
...
<div class='row'>
	<div class='col-xs-12'>
		...
		$sql = dbQuery("SELECT tbl_prepare.id_order, tbl_prepare.id_employee FROM tbl_prepare LEFT JOIN tbl_order ON tbl_prepare.id_order = tbl_order.id_order WHERE  current_state = 4 and tbl_prepare.id_employee = $user_id");
		$n = 1;
