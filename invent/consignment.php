    ...
    <div class="col-xs-6">
	   ...
	   if(isset($_GET['view_detail'])&&isset($_GET['id_order'])){
		   ...
		   if($order->valid==1 || $order->current_state !=1 && $order->current_state !=3){ $active = "style='display:none;'";}else{$active = ""; }
		    echo"
		   <li><a href='index.php?content=consignment' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link'><span class='glyphicon glyphicon-arrow-left' style='color:#5cb85c; font-size:30px;'></span><br />กลับ</button></a></li>
       		<li $can_edit ><a href='index.php?content=consignment&add=y&id_order=$id_order' style='text-align:center; background-color:transparent;' $active ><button type='button' class='btn btn-link'  $active><span class='glyphicon glyphicon-pencil' style='color:#5cb85c; font-size:30px;'></span><br />แก้ไข</button></a></li>";
			}else if(isset($_GET['edit']) || isset($_GET['add'])){
		   echo"
		   <li><a href='index.php?content=consignment' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link'><span class='glyphicon glyphicon-arrow-left' style='color:#5cb85c; font-size:30px;'></span><br />กลับ</button></a></li>";
		    if(isset($_GET['id_order'])){$id_order = $_GET['id_order'];
		  echo"	<li><a href='controller/consignmentController.php?save_order&id_order=$id_order' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link' onclick='edit_stock()'><span class='glyphicon glyphicon-floppy-disk' style='color:#5cb85c; font-size:30px;'></span><br />บันทึก</button></a></li>";}
	  		}else{
		   echo"
		   <li $can_add><a href='controller/consignmentController.php?check_add' style='text-align:center; background-color:transparent; padding-bottom:0px;' ><button type='button' class='btn btn-link' ><span class='glyphicon glyphicon-plus-sign' style='color:#5cb85c; font-size:30px;'></span><br />$page_name</button></a></li>";
	   }
	   ?>
       </ul>
    </div>
...
echo"<form id='add_order_form' action='controller/consignmentController.php?add=y' method='post'>
	...
	<div class='col-xs-3'><input type='checkbox' id='auto_zone' name='auto_zone' $active/><label for='auto_zone' style='margin-left:10px;'>สร้างโซนอัตโนมัติ</label></div>
	</div>
	...
	//////////////////// ได้ออเดอร์ ID แล้ว
	if(isset($_GET['id_order'])){ 
		echo"<form id='add_detail_form' action='controller/consignmentController.php?add&insert_detail' method='post'>
	...
	<hr style='border-color:#CCC; margin-top: 15px; margin-bottom:15px;' />";
	//order_grid($customer->id_customer, $order->id_order, "controller/consignmentController.php?add_to_order");	
	//*********************************  เริ่ม ORDER GRID ******************************************//
	echo"
	<div class='row'>
...
//*********************************************** แก้ไขออเดอร์ **************************************************************//
	...
	echo"		
        <div class='row'>
        	<div class='col-xs-12'><h4>".$order->reference." - ".$customer->full_name."<p class='pull-right'>พนักงาน : &nbsp;".$sale->full_name."</p></h4></div>
        </div>
		<hr style='border-color:#CCC; margin-top: 0px; margin-bottom:15px;' />
		<div class='row'>
		<div class='col-xs-12'>
		...
		<div class='row'>"; echo $order->orderProductTable(); echo "</div><div class='row'><div class='col-lg-12'><p><h4>ข้อความ :  "; if($order->comment ==""){ echo"ไม่มีข้อความ";}else{ echo $order->comment; }echo "</h4></p></div></div><h4></h4></form>";
//*********************************************** จบหน้าแก้ไข ****************************************************//
}else{
	//************************************************ แสดงรายการ *************************************************//
...
<div class='col-xs-12'>
	<table class='table'>
		...
		echo"<tr style='color:#FFF; background-color:".state_color($current_state).";'>
				...
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=consignment&id_order=$id_order&view_detail=y'\">$reference</td>
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=consignment&id_order=$id_order&view_detail=y'\">$cus_first_name $cus_last_name</td>
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=consignment&id_order=$id_order&view_detail=y'\">$employee_name</td>
				...
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=consignment&id_order=$id_order&view_detail=y'\">"; echo thaiDate($date_add)."</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=consignment&id_order=$id_order&view_detail=y'\">"; echo thaiDate($date_upd)."</td>
			</tr>";
		$i++;
		}
		}else if($row==0){
