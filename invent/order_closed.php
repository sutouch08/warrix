...
<div class="row">
    ...
    <div class="col-sm-6">
	 ...
	 if(isset($_GET['view_detail'])&&isset($_GET['id_order'])){
		   echo"
		   <li><a href='index.php?content=order_closed' style='text-align:center; background-color:transparent;'><button type='button' class='btn btn-link'><span class='glyphicon glyphicon-arrow-left' style='color:#5cb85c; font-size:30px;'></span><br />กลับ</button></a></li>";
	   }
...
if(isset($_GET['view_detail'])&&isset($_GET['id_order'])){
	...
	echo "
		...
		<p class='pull-right' $confirm>
			<a href='controller/billController.php?confirm_order&id_order=$id_order&id_employee=$id_employee' >
...
$paginator->Per_Page("tbl_order","WHERE current_state =9",$get_rows);
...
<div class='col-sm-12'>
	<table class='table table-striped'>
		...
		$result = dbQuery("SELECT id_order,reference,date_add,date_upd,payment,id_customer,id_employee,current_state FROM tbl_order WHERE current_state = 9 ORDER BY id_order DESC LIMIT $Page_Start , $Per_Page");
		$i=0;
		$n = 1;
	...
	echo"<tr>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$n</td>
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$reference</td>
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$customer_name</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">"; echo number_format($amount)."</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$payment</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$status</td>
				<td style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">$employee_name</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">"; echo thaiDate($date_add)."</td>
				<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=order_closed&id_order=$id_order&view_detail=y'\">"; echo thaiDate($date_upd)."</td>
			</tr>";
			$i++; $n++;
		}
		}else if($row==0){
