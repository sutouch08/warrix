...
if(isset($_GET['product_name'])&&isset($_REQUEST['term'])){
	if($_REQUEST['term'] =="*"){
		...
		$qstring = "SELECT id_product, product_code, product_name FROM tbl_product WHERE product_code LIKE '%".$_REQUEST['term']."%' OR product_name LIKE'%".$_REQUEST['term']."%'";
	}
	$result = dbQuery($qstring);//query the database for entries containing the term
if ($result->num_rows>0)
	{
	...
	foreach ($id_product_attribute as $id ){	
		if($order_qty[$i] !=""){
			...
			if(!ALLOW_UNDER_ZERO){
				...
				$instock = ($product->available_qty() + $qty_moveing) - $product->order_qty(); 
...
if(isset($_GET['edit_discount'])){
	...
	dbQuery("INSERT INTO tbl_discount_edit(id_order,id_employee)values('$id_order','$id_employee')");
	list($id_discount_edit) = dbFetchArray(dbQuery("SELECT MAX(id_discount_edit) FROM tbl_discount_edit WHERE id_order = '$id_order'"));
	$i = 0;
	foreach ($id_order_detail_array as $id ){
		$reduction_edit = $reduction_array[$i];
		//echo "$id$reduction";
		list($reduction_percent,$reduction_amount,$product_qty,$product_price,$id_product_attribute) = dbFetchArray(dbQuery("SELECT reduction_percent,reduction_amount,product_qty,product_price,id_product_attribute FROM tbl_order_detail WHERE id_order_detail = '$id'"));
		...
		if($reduction_edit == "$reduction"){
		}else{
			if($unit == "%"){
				$discount = ($product_price*$reduction_edit)/100;
				$dis = $discount * $product_qty;
				$final_price = $product_price - $discount;
				$total_amount = $final_price * $product_qty;
				dbQuery("UPDATE tbl_order_detail SET reduction_percent = '$reduction_edit',final_price = '$final_price',total_amount = '$total_amount',discount_amount = '$dis' WHERE id_order_detail = '$id'");
				dbQuery("INSERT INTO tbl_discount_edit_detail(id_discount_edit, id_order_detail ,dis_before ,dis_after)values('$id_discount_edit','$id','$reduction','$reduction_edit')");
				dbQuery("UPDATE tbl_order_detail_sold SET reduction_percent = '$reduction_edit',final_price = '$final_price',total_amount = '$total_amount',discount_amount = '$dis' WHERE id_order = $id_order AND id_product_attribute = $id_product_attribute");
			}else if($unit == "฿"){
				$dis = $reduction * $product_qty;
				$final_price = $product_price - $reduction;
				$total_amount = $final_price * $product_qty;
				dbQuery("UPDATE tbl_order_detail SET reduction_amount = '$reduction_edit',final_price = '$final_price',total_amount = '$total_amount',discount_amount = '$dis' WHERE id_order_detail = '$id'");
				dbQuery("INSERT INTO tbl_discount_edit_detail(id_discount_edit,id_order_detail,dis_before,dis_after)values('$id_discount_edit','$id','$reduction','$reduction_edit')");
				dbQuery("UPDATE tbl_order_detail_sold SET reduction_amount = '$reduction_edit',final_price = '$final_price',total_amount = '$total_amount',discount_amount = '$dis' WHERE id_order = $id_order AND id_product_attribute = $id_product_attribute");
			}
		}
		$i++;
		echo "1";
