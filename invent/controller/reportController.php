...
if(isset($_GET['fifo_report'])&&isset($_GET['product'])&&isset($_GET['view'])){	
	...
	$report_title = "รายงานความเคลื่อนไหวสินค้า วันที่ ".thaiTextDate($from)."  ถึง ".thaiTextDate($to);
	...
	$header_row = "
	<tr>
	<td width='10%'>วันที่ </td><td width='15%'>เลขที่เอกสาร</td><td width='10%'>คลัง</td><td width='10%' align='right'>เข้า</td><td width='10%'  align='right'>ออก</td><td width='10%'  align='right'>ต้นทุน</td>
	<td width='10%'  align='right'>ต้นทุนรวม</td><td width='10%'  align='right'>คงเหลือ</td><td width='15%'  align='right'>มูลค่า</td>
	</tr>";
	
		$qr = dbQuery("SELECT id_product_attribute FROM tbl_stock_movement WHERE $product  AND $warehouse $view GROUP BY id_product_attribute");
		$row = dbNumRows($qr); 
		...
		while($i<$row){
			...
			$sql = dbQuery("SELECT date_upd, reference, sum(move_in), sum(move_out), id_warehouse FROM tbl_stock_movement WHERE id_product_attribute = $id_product_attribute AND $warehouse $view GROUP BY reference, id_warehouse ORDER BY date_upd ASC");
				$rows = dbNumRows($sql);
				$v = 0;
				list($stock_in, $stock_out) = dbFetchArray(dbQuery("SELECT SUM(move_in) AS stock_in, SUM(move_out) AS stock_out FROM tbl_stock_movement WHERE id_product_attribute = $id_product_attribute AND $warehouse AND date_upd < '$from'"));
				$bf_balance = $stock_in-$stock_out;
				$html .="<tr><td colspan='8' style='vertical-align:middle;'>$reference  :  $product_name  :  $barcode </td><td style='text-align:right; vertical-align:meddle;'>
			<a href='controller/reportController.php?print_stock_card&id_product_attribute=$id_product_attribute&from_date=$from&to_date=$to&before_date=$before_date&id_warehouse=$id_warehouse&title=$report_title' target='_blank'>
			<button type='button' class='btn btn-primary'><span class='glyphicon glyphicon-print' style='color:#FFF; '></span></button>
			</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		</td></tr>";
				$html .= $header_row;
				$html .="<tr style='font-size: 12px;'><td>".thaiDate($before_date)."</td><td colspan='6'>ยอดยกมา</td>
				<td  align='right'>".number_format($bf_balance)."</td><td  align='right'>".number_format($bf_balance*$product_cost,2)."</td></tr>";
				$balance = $bf_balance;
				$total_in = 0;
				$total_out = 0;
				while($v<$rows){
				list($date_upd, $document, $move_in, $move_out, $wh_id) = dbFetchArray($sql);
				$balance = $balance+$move_in-$move_out;		
				$total_in = $total_in + $move_in;
				$total_out = $total_out + $move_out;
				$html .=" <tr style='font-size: 12px;'><td>".thaiDate($date_upd)."</td><td>$document</td><td>".get_warehouse_name_by_id($wh_id)."</td><td  align='right'>$move_in</td><td  align='right'>$move_out</td>
				<td  align='right'>$product_cost</td><td  align='right'>"; if($move_in !=0){ $html .= number_format($move_in*$product_cost,2);} else { $html .= number_format($move_out*$product_cost,2);} $html .="</td>
				<td  align='right'>$balance</td><td  align='right'>".number_format($balance*$product_cost,2)."</td></tr>";
				$v++;
				}
				$move_ment = $total_in - $total_out;
				$html .= "<tr style='font-size: 12px;'><td colspan='5' style='vertical-align:middle;'>&nbsp;</td><td align='right'>รวมเข้า</td><td align='right'>รวมออก</td><td align='right'>เคลื่อนไหว</td><td align='right'>มูลค่า</td></tr>
						<tr style='font-size: 12px;'><td colspan='5' align='right'>&nbsp;</td><td align='right'>$total_in</td><td align='right'>$total_out</td><td align='right'>$move_ment</td>
						<td align='right'>".number_format($move_ment*$product_cost,2)."</td></tr>
						<tr style='font-size: 12px;'><td colspan='9'><h4>&nbsp;</h4></td></tr>";
				$i++;
		}
		}else{
...
/***********************  รายงานความเคลื่อนไหวสินค้าแต่ละตัว export to excel *************************************/
if(isset($_GET['export_fifo_report'])&&isset($_GET['product'])&&isset($_GET['view'])){	
	$warehouse_rank = $_GET['warehouse'];
	$product_rank = $_GET['product'];
	$view_rank = $_GET['view'];	
	$today = date('Y-m-d');
	...
	$line = array(1=>array("------------------------------------------------------------------------------------------------------------------------------------"));
	$sub_header = array("วันที","เลขที่เอกสาร","คลัง","เข้า","ออก","ต้นทุน","ต้นทุนรวม","คงเหลือ","มูลค่า");
	$body = array();
	$space = array("----------------------------------------------------------------------------------------------------------------------------------");
		$qr = dbQuery("SELECT id_product_attribute FROM tbl_stock_movement WHERE $product  AND $warehouse $view GROUP BY id_product_attribute");
		$row = dbNumRows($qr); 
		...
		while($i<$row){
			...
			$sql = dbQuery("SELECT date_upd, reference, sum(move_in), sum(move_out), id_warehouse FROM tbl_stock_movement WHERE id_product_attribute = $id_product_attribute AND $warehouse $view GROUP BY reference, id_warehouse ORDER BY date_upd ASC");
				...
				list($stock_in, $stock_out) = dbFetchArray(dbQuery("SELECT SUM(move_in) AS stock_in, SUM(move_out) AS stock_out FROM tbl_stock_movement WHERE id_product_attribute = $id_product_attribute AND $warehouse AND date_upd < '$from'"));
				$bf_balance = $stock_in-$stock_out;
				$header = array("$reference  :  $product_name  :  $barcode");
				array_push($body, $header);
				array_push($body, $sub_header);
				$bal = array(thaiDate($before_date),"ยอดยกมา"," "," "," "," "," ",number_format($bf_balance), number_format($bf_balance*$product_cost,2));
				array_push($body, $bal);
				$balance = $bf_balance;
				while($v<$rows){
				list($date_upd, $document, $move_in, $move_out, $wh_id) = dbFetchArray($sql);
				$balance = $balance+$move_in-$move_out;		
				if($move_in !=0){ $move = number_format($move_in*$product_cost,2);} else { $move = number_format($move_out*$product_cost,2);}
				$data = array(thaiDate($date_upd), $document, get_warehouse_name_by_id($wh_id), $move_in, $move_out, $product_cost, $move, $balance, number_format($balance*$product_cost,2));
				array_push($body, $data);
				$v++;
				}
				array_push($body, $space);
				$i++;
		}
		}else{
		...
		$sheet_name = "Stock_Movement_Report";
		$xls = new Excel_XML('UTF-8', false, $sheet_name); 
		$xls->addArray($title);
		$xls->addArray($line);
		$xls->addArray ( $body ); 
		$xls->generateXML("Stock_movement_report"); 
}
...
//****************************  รายงานสินค้าคงเหลือ ****************************************//
	...
	/////////////////////////////////////////////////////////////////////
	$report_title = "รายงานสินค้าคงเหลือ ณ วันที่ ".thaiTextDate($date); if($warehouse ==""){ $report_title .=" รวมคลังทุกคลัง";}else{ $report_title .="  คลัง : ".getWarehouseName($warehouse_selected);}
	...
	<thead>
		<th style='width:5%; text-align:center;'>ลำดับ</th><th style='width:10%;'>บาร์โค้ด</th><th style='widht:15%;;'>รหัส</th><th style='widht:20%;'>ชื่อสินค้า</th>
		<th style='widht:10%; text-align: right;'>ราคาทุน</th><th style='widht:10%; text-align: right;'>คงเหลือ</th><th style='widht:10%; text-align: right;'>มูลค่า</th>
	</thead>";
	
	$qr = dbQuery("SELECT tbl_stock.id_product_attribute, SUM(qty) FROM tbl_stock LEFT JOIN tbl_product_attribute ON tbl_stock.id_product_attribute = tbl_product_attribute.id_product_attribute LEFT JOIN 
							tbl_zone ON tbl_stock.id_zone = tbl_zone.id_zone WHERE $product  AND $warehouse  GROUP BY id_product_attribute");
		$row = dbNumRows($qr); 
		$total_qty = 0;
		$total_amount = 0;
		$i = 0;
		...
		while($i<$row){
			...
			if($view_rank==1){
				...
				$sql .=" WHERE id_product_attribute = $id_product_attribute AND $warehouse AND date_upd BETWEEN '$date_selected' AND '$today' GROUP BY id_product_attribute";
				//echo $sql;
				list($stock_in, $stock_out) = dbFetchArray(dbQuery($sql));
				$stock = ($qty + $stock_moveing) + $stock_out - $stock_in;
				}else	if($view_rank==0){ 
					$stock = $qty+$stock_moveing;
				}
			if($stock <1){ 
			$amount = 0;
			 }else{
			$amount = $stock * $product->product_cost;
			$html .= "<tr style='font-size: 12px;'><td align='center'>".$n."</td><td>".$product->barcode."</td><td>".$product->reference."</td><td>".$product->product_name."</td>
			<td align='right'>".number_format($product->product_cost,2)."</td><td align='right'>".number_format($stock)."</td><td align='right'>".number_format($amount,2)."</td></tr>";
			$n++;
			}
...
//****************************  รายงานสินค้าคงเหลือ  export to excel ****************************************//
	...
	if($product_rank==0){  //// product
		...
		if($warehouse_rank == 0){
				...
				$id_warehouse = "";
					...
					$warehouse_selected = $_GET['warehouse_selected'];
					$warehouse = "WHERE (tbl_zone.id_warehouse = '$warehouse_selected') ";   
					$id_warehouse = "AND id_warehouse = $warehouse_selected";    
				}
		}else if($product_rank==1){ 
			$pro_from = $_GET['product_from'];
			$pro_to = $_GET['product_to'];
			if($pro_from > $pro_to){ 	$product_from = $pro_to; $product_to = $pro_from; }else{ $product_from = $pro_from; $product_to = $pro_to; }
			$product ="WHERE (tbl_product_attribute.id_product BETWEEN $product_from AND $product_to)";
			if($warehouse_rank == 0){
				$warehouse = "";
				$id_warehouse = "";
				}else if($warehouse_rank==1){
					$warehouse_selected = $_GET['warehouse_selected'];
					$warehouse = "AND (tbl_zone.id_warehouse = '$warehouse_selected') ";
					$id_warehouse = "AND id_warehouse = $warehouse_selected";
				}
			}else if($product_rank ==2){
				$product_selected = $_GET['product_selected'];
				$product = "WHERE (tbl_product_attribute.id_product = $product_selected)";
				if($warehouse_rank == 0){
				$warehouse = "";
				$id_warehouse = "";
				}else if($warehouse_rank==1){
	...
	$sub_header = array("ลำดับ่","บาร์โค้ด","รหัส","ขื่อสินค้า","ราคาทุน","คงเหลือ","มูลค่า");
	$line = array(1=>array("======================================================================================="));
	$body = array();
	$qr = dbQuery("SELECT tbl_stock.id_product_attribute, barcode, reference, product_name, cost, sum(qty) as qty FROM tbl_stock LEFT JOIN tbl_product_attribute ON tbl_stock.id_product_attribute = tbl_product_attribute.id_product_attribute LEFT JOIN tbl_product ON tbl_product_attribute.id_product = tbl_product.id_product LEFT JOIN tbl_zone ON tbl_stock.id_zone = tbl_zone.id_zone LEFT JOIN tbl_warehouse ON tbl_zone.id_warehouse = tbl_warehouse.id_warehouse $product  $warehouse  GROUP BY barcode");
		$row = dbNumRows($qr); 
		$total_qty = 0;
		$total_amount = 0;
		$i = 0;
		...
		array_push($body, $sub_header);
		while($i<$row){
			list($id_product_attribute, $barcode, $reference, $product_name, $product_cost, $qty) = dbFetchArray($qr);
			$stock_moveing = stock_moveing($id_product_attribute);
			if($view_rank==1){
				$sql = "SELECT SUM(move_in) AS stock_in, SUM(move_out) AS stock_out FROM tbl_stock_movement";
				$sql .=" WHERE id_product_attribute = $id_product_attribute $id_warehouse AND date_upd BETWEEN '$date_selected' AND '$today' GROUP BY id_product_attribute";
				list($stock_in, $stock_out) = dbFetchArray(dbQuery($sql));
				$stock = ($qty + $stock_moveing) + $stock_out - $stock_in;
				}else	if($view_rank==0){ $stock = $qty+$stock_moveing;}
			$amount = $stock * $product_cost;
			$arr = array($n, $barcode, $reference, $product_name, number_format($product_cost,2), number_format($stock), number_format($amount,2));
			array_push($body, $arr);
			$total_qty = $total_qty + $stock;
			$total_amount = $total_amount + $amount;
			$i++;
...
if(isset($_GET['sale_report_zone'])&&isset($_GET['from_date'])&&isset($_GET['to_date'])){
	...
	$title = "รายงานยอดขายแยกตามพื้นที่การขาย วันที่ ".thaiTextDate($from)." ถึง ".thaiTextDate($to);
...
if(isset($_GET['sale_report_employee'])&&isset($_GET['from_date'])&&isset($_GET['to_date'])){
	...
	$title = "รายงานยอดขายแยกตามพนักงานขาย วันที่ ".thaiTextDate($from)." ถึง ".thaiTextDate($to);
		$header = array(1=>array($title));
...
if(isset($_GET['sale_by_customer'])&&isset($_GET['view'])&&isset($_GET['customer'])){
	...
	$report_title = "รายงานยอดขาย แยกตามลูกค้า วันที่ ".$rank;
...
if(isset($_GET['sale_report_product'])&&isset($_GET['from_date'])&&isset($_GET['to_date'])){
	...
	$title = "รายงานยอดขายแยกตามสินค้า วันที่ ".thaiTextDate($from)." ถึง ".thaiTextDate($to);
		$header = array(1=>array($title));
...
if(isset($_GET['recieved_report'])&&isset($_GET['view'])&&isset($_GET['product'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานการรับสินค้า วันที่ ".$rank;
	$html = " 	<h4 align='center'>$report_title</h4> <hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />
	<table class='table table-striped table-hover'>
		<thead>
...
if(isset($_GET['request_report'])){
	...
	//**************************  report  ************************//
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานการร้องขอสินค้า วันที่ ".$rank;
	$html = "<h4 align='center'>$report_title</h4><hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />";
...
//*************************************** รายงานยอดขาย แยกตามเลขที่เอกสาร  *******************************************//
	if(isset($_GET['sale_by_document'])&&isset($_GET['view'])){
		...
		switch($view){
			...
			$html = "<div class='col-lg-12'><h4>รายงานยอดขายแยกตามเอกสาร $rank</h4></div><div class='col-lg-12'><hr style='border-color:#CCC; margin-top: 0px; margin-bottom:15px;' /></div>
			<table class='table table-striped'><thead><th style='width:10%; text-align:center;'>ลำดับ</th><th style='width:10%;'>เอกสาร</th><th style='width:10%; text-align:center;'>วันที่</th>
				<th style='width:35%;'>ลูกค้า</th><th style='width:20%; text-align:center;'>มูลค่า</th></thead>";
			...
			if($row>0){
				...
				while($i<$row){
					...
					//$sale_amount = $total_amount-$vat;
					$html .="<tr><td align='center'>$n</td><td>".$reference."</td><td align='center'>".thaiDate($date_upd)."</td><td>".$customer_name."</td>".
					//<td style='text-align: right; padding-right: 5px;'>".number_format($sale_amount,2)."</td><td style='text-align: right; padding-right: 5px;'>".number_format($vat,2)."</td>
					"<td style='text-align: right; padding-right: 5px;'>".number_format($total_amount,2)."</td></tr>";
					$total_movement += $total_amount;
					$i++;
					$n++;
				}
				//$total_vat = $total_movement*0.07;
				//$total_sale = $total_movement - $total_vat;
				$html .="<tr><td colspan='4' style='text-align: right; padding-right:10px; '><h4>รวม</h4></td>".
				//<td style='text-align: right; padding-right: 5px;'><h4>".number_format($total_sale,2)."</h4></td><td style='text-align: right; padding-right: 5px;'><h4>".number_format($total_vat,2)."</h4></td>
				"<td style='text-align: right; padding-right: 5px;'><h4>".number_format($total_movement,2)."</h4></td></tr></table>";
			}else{
				$html .="<tr><td colspan='5' style='text-align:center;'><h4>ไม่มีรายการตามช่วงที่เลือก</h4></td></tr></table>";
			}
			echo $html;
...
//*************************************** รายงานยอดขาย แยกตามเลขที่เอกสาร Export to excel  *******************************************//
	if(isset($_GET['export_sale_by_document'])&&isset($_GET['view'])){
		...
		switch($view){
			...
			$report_title = "รายงานยอดขายแยกตามเอกสาร ".$rank;
			$title = array(1=>array($report_title));
			$sub_header = array("ลำดับ","เอกสาร","วันที่","ลูกค้า","มูลค่า","ภาษีมูลค่าเพิ่ม","มูลค่ารวม");
			$body = array();
			$line = array(1=>array("==========================================================="));
			$sql = dbQuery("SELECT id_order, reference, id_customer, date_upd FROM tbl_order WHERE role = 1 AND current_state = 9 $views ORDER BY reference ASC");
			$row = dbNumRows($sql);
			if($row>0){
				...
				while($i<$row){
					...
					$vat = $total_amount*0.07;
					$sale_amount = $total_amount-$vat;
					$arr = array($n, $reference, thaiDate($date_upd), $customer_name, number_format($sale_amount,2), number_format($vat,2), number_format($total_amount,2));
					array_push($body, $arr);
					$total_movement += $total_amount;
					$i++;
					$n++;
				...
				$total_vat = $total_movement*0.07;
				$total_sale = $total_movement - $total_vat;
				$arr = array("","","","รวม", number_format($total_sale,2), number_format($total_vat,2), number_format($total_movement,2) );
				array_push($body, $arr);
			}else{
...
if(isset($_GET['export_request_by_customer'])&&isset($_GET['id_product'])){
	...
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานการร้องขอสินค้า แยกตามลูกค้า วันที่ ".$rank;
	$title = array(1=>array($report_title));
...
if(isset($_GET['customer_by_product_attribute'])&&isset($_GET['view'])&&isset($_GET['product'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานลูกค้าแยกตามรายการสินค้า วันที่ ".$rank;
...
if(isset($_GET['product_by_customer'])&&isset($_GET['view'])&&isset($_GET['customer'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานสินค้า แยกตามลูกค้า วันที่ ".$rank;
	$html = " 	<h4 align='center'>$report_title</h4> <hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />
	<table class='table table-striped table-hover'>";
...
if(isset($_GET['product_attribute_by_customer'])&&isset($_GET['view'])&&isset($_GET['customer'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานรายการสินค้า แยกตามลูกค้า วันที่ ".$rank;
	$html = " 	<h4 align='center'>$report_title</h4> <hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />
	<table class='table table-striped table-hover'>";
...
if(isset($_GET['document_by_product_attribute'])&&isset($_GET['view'])&&isset($_GET['product'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานเอกสาร แยกตามรายการสินค้า วันที่ ".$rank;
	$html = " 	<h4>$report_title</h4> <hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />
	<table class='table table-striped table-hover'>
		<thead>
...
if(isset($_GET['sale_amount_document'])&&isset($_GET['view'])&&isset($_GET['sale'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." &nbsp;ถึง &nbsp;".thaiDate($to); }
	$report_title = "รายงานยอดขาย &nbsp;  วันที่ &nbsp;".$rank;
	$html = "	<table class='table table-striped table-hover'>";
	$header = "
		<tr>
			<td style='align:center; width:10%;'>วันที่</td><td style='align:center; width:10%;'>เอกสาร</td><td style='align:center; width:25%;'>ลูกค้า</td>
			<td style='text-align:right; width:15%;'>จำนวนเงิน</td><td style='text-align: right; width:15%;'>ส่วนลด(มูลค่า)</td><td style='text-align: right; width:15%;'>สุทธิ</td><td style='text-align:center; width:10%;'>พนักงาน</td>
		</tr>";
		$sql = dbQuery("SELECT id_sale, id_employee FROM tbl_sale WHERE $sale");
		$rs = dbNumRows($sql);
		if($rs>0){
			...
			while($r<$rs){
				...
				$html .= "<tr><td colspan='10' align='center'><h4>$report_title &nbsp;&nbsp; พนักงาน : $sale_name </h4></td></tr>";
				$html .= $header;
				$qr = dbQuery("SELECT  reference, id_customer, SUM(discount_amount), SUM(total_amount), date_upd   FROM tbl_order_detail_sold WHERE id_sale = $id_sale  $view AND id_role IN(1,5) GROUP BY id_order ORDER BY date_upd ASC");
				$row = dbNumRows($qr); 
				$i = 0;
				$move_price = 0; $move_amount = 0; $discount =0;
					while($i<$row){
						list($reference, $id_customer, $discount_amount, $total_amount, $date_upd) = dbFetchArray($qr);
						$total_price = $total_amount + $discount_amount;
						$customer = new customer($id_customer);
						$customer_name = $customer->full_name;
						$date = thaiDate($date_upd);
						$html .=" <tr style='font-size:12px;'><td>$date</td><td>$reference</td><td>$customer_name</td><td align='right'>".number_format($total_price,2)."</td>
						<td align='right'>".number_format($discount_amount,2)."</td><td align='right'>".number_format($total_amount,2)."</td><td align='center'>$sale_first_name</td></tr>";
						$discount += $discount_amount;
				...
				$html .= "<tr><td colspan='3' align='right'><b>รวม &nbsp; </b></td><td align='right'><b>".number_format($move_price,2)."</b></td>
							<td align='right'><b>".number_format($discount,2)."</b></td><td align='right'><b>".number_format($move_amount,2)."</b></td><td> &nbsp;</td></tr>";
...
if(isset($_GET['export_sale_amount_document'])&&isset($_GET['view'])&&isset($_GET['sale'])){
	...
	$report_title = "รายงานยอดขาย "." "." "."  วันที่ "." "." ".$rank;
	$title = array(1=>array($report_title));
	$sub_header = array("วันที่", "เอกสาร","ลูกค้า", "จำนวนเงิน","ส่วนลด(มูลค่า)","สุทธิ","พนักงาน");
	$body = array();
	$line = array(1=>array("==========================================================="));
		$sql = dbQuery("SELECT id_sale, id_employee FROM tbl_sale WHERE $sale");
		$rs = dbNumRows($sql);
			...
			while($r<$rs){
				...
				$move_price = 0; $move_amount = 0; $discount =0;
					while($i<$row){
						...
						$customer_name = $customer->full_name;
						$date = thaiDate($date_upd);
						$arr = array($date, $reference, $customer_name, number_format($total_price,2), number_format($discount_amount,2), number_format($total_amount,2), $sale_first_name);
						array_push($body, $arr);
						$discount += $discount_amount;
						$move_price += $total_price;
						$move_amount += $total_amount;
					...
					$arr = array("","","รวม",number_format($move_price,2), number_format($discount,2), number_format($move_amount,2));
					array_push($body, $arr);
					$arr = array("----------------------------------------------------------------------------------------------------------------------");
					array_push($body, $arr);
					$arr = array("");
...
if(isset($_GET['document_by_customer'])&&isset($_GET['view'])&&isset($_GET['customer'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานเอกสาร แยกตามลูกค้า วันที่ ".$rank;
	$html = " 	<h4 align='center'>$report_title</h4> <hr style='border-color:#CCC; margin-top: 0px; margin-bottom:0px;' />
	<table class='table table-striped table-hover'>";
...
if(isset($_GET['stock_back_log'])&&isset($_GET['view'])&&isset($_GET['customer'])&&isset($_GET['product'])){
	...
	/////////////////////////////////////////////////////////////////////
	if($view_rank ==0){ $rank = " ทั้งหมด"; }else{ $rank = thaiDate($from)." ถึง ".thaiDate($to); }
	$report_title = "รายงานสินค้าค้างส่ง วันที่ ".$rank;
