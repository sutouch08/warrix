...
///////////////////// จบหน้าเพิ่ม //////////////////
	...
	//////////////////////   รายละเอียด  ////////////////////
	$id_group = $_GET['id_group'];
	$customer = customerTableBYGroup($id_group);
	$list = customerTableBYGroup($id_group);
	$member = dbNumRows(dbQuery("SELECT * FROM tbl_customer_group WHERE id_group = $id_group"));
	$data = dbFetchArray($customer);
	$row = dbNumRows($customer);
	$i=0;
	if($data['show_price']==1){ $show_price = "แสดงราคา";}else{ $show_price= "ไม่แสดงราคา";}
	echo"<div class='row'>
			<div class='col-sm-12'>
				<div class='alert alert-info'>
				<p>ชื่อกลุ่ม : ".$data['group_name']."</p>
					<p>แสดงราคา : ".$show_price."</p>
				...
				if($row>0){
					while($i<$row){
						list($id_group, $id_customer, $first_name, $last_name, $email, $date_add, $group_name, $show_price) = dbFetchArray($list);
						echo"
					<tr>
					...
					<td style='cursor:pointer;' onclick=\"document.location='index.php?content=customer&view_detail=y&id_customer=$id_customer'\">$email</td>
					<td align='center' style='cursor:pointer;' onclick=\"document.location='index.php?content=customer&view_detail=y&id_customer=$id_customer'\">". thaiTextDate($date_add)."</td>
					</tr>";
					$i++;
					}
				}else{
