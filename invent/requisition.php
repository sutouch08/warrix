...
//	if(isset($_GET['role'])){ $role = $_GET['role']; }else{ $role = 2;  }
	///--------------- เบิกสินค้า
	?>
<div class="container">
<!-- page place holder -->
...
$customer_name = $customer->full_name; 
	...
	$id_customer = "";
	$customer_name = ""; 
	$new_ref = get_max_role_reference("PREFIX_REQUISITION",$role);
}
echo"<form id='add_order_form' action='controller/requisitionController.php?add=y' method='post'>
	...
	<div class='col-xs-3'><div class='input-group'><span class='input-group-addon'>วัตถุประสงค์</span>
							<select name='role' id='role' class='form-control' $active>
								<option value='2'"; if($role==2){ echo"selected='selected'";} echo">เบิกเพื่อใช้งาน</option>
								<option value='6'"; if($role==6){ echo"selected='selected'";} echo">เบิกไปแปรรูป</option>
							</select>
							</div> </div>
	</div>
...
//*********************************************** แก้ไขออเดอร์ **************************************************************//
	...
	echo"		
        <div class='row'>
        	<div class='col-xs-12'><h4>".$order->reference." - เบิกสินค้า<p class='pull-right'>ผู้เบิก : &nbsp;".$employee->full_name."</p></h4></div>
        </div>
		<hr style='border-color:#CCC; margin-top: 0px; margin-bottom:15px;' />
		<div class='row'>
		<div class='col-xs-12'>
