...
$(document).ready(function() {
	$("#zone_name").keyup(function(){
		...
		$.ajax({
			...
			cache:"false", data:"zone_name="+zone_name+"&id_warehouse="+warehouse, 
...
$(document).ready(function() {
	$("#barcode_zone").keyup(function(){
		...
		$.ajax({
			type:"GET",
			url:"controller/zoneController.php",
			cache:"false", data:"barcode_zone="+barcode_zone+"&id_warehouse="+warehouse, 
			success: function(msg){
...
function validate(){
	...
	}else if(checked_code ==""){
		return("โซนของคุณไม่ได้ระบุบาร์โค้ด ต้องการดำเนินการต่อหรือไม่");
	}else{
		if(checked_name == 1){
			alert("ชื่อโซนซ้ำ ไม่อณุญาติให้ใช้ชื่อซ้ำกันภายในคลังเดียวกัน");
		}else if(checked_code == 1){
